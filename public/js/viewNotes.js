...
const renderDataAsHtml = (data) => {
  ...
  for (const noteItem in data) {
    ...
    if (window.location.pathname === "/viewArchievedNotes.html") {
        if (note.archived === true) {
            cards += createArchivedCard(note, noteItem)
        }
        
    }
    if (window.location.pathname === "/viewNotes.html") {
        if (!note.archived === true) {
...
const createArchivedCard = (note, noteId) => {
  return `
    <div class="column is-one-quarter">
      <div class="card">
        <header class="card-header">
...
const archiveNote = (noteId) => {
    console.log("archiveNote called")
    const noteEdits = {
        archived: true
    }
...
const saveEditedNote = () => {
    console.log("SaveEditedNote called");
